...
inline void Graph_Matrix_3D<node_dtype, dimen_t>::resize(
    const dimen_t newX, const dimen_t newY, const dimen_t newZ,
    const Init_Func &initialiser, RESIZE_TYPE resize_type) {
...
inline void Graph_Matrix_3D<node_dtype, dimen_t>::resize(
    ...
    if (newX == this->total_abs.mX && newY == this->total_abs.mY &&
        newZ == this->total_abs.mZ)
        return;
    ...
    if (resize_type == RESIZE_TYPE::MANUAL) {
        if (was_auto_expanding)
            this->pause_auto_expansion(); // if auto_expansion is on, pause it
    }

    /**
     * @note - Above code maybe a bit confusing... the "initialiser function" is
     *decided by the add_* or push_* functions themselves, BASED on
     *`curr_resize_type`, that is why we stored it
     */

    // the flag has been used to `alternately` pop or push layers, from the both
    // parts of the axis
    bool flag = false;

    while (this->total_abs.mX > newX) {
        flag ? this->pop_xplus_layer() : // planes intersected by the +ve x axis
    ...
    while (this->total_abs.mY > newY) {
        flag ? this->pop_yplus_layer() : this->pop_yminus_layer();

        flag = !flag;
    }

    while (this->total_abs.mZ > newZ) {
        flag ? this->pop_zplus_layer() : this->pop_zminus_layer();
    ...
    auto org_t_abs{
        this->total_abs
            .mX}; // since total_abs will be modified by the next operations, we
                  // need to store the ORIGINAL value of the total abs values
    if (this->total_abs.mX < newX) {
        if (tmp_resize_data.curr_resize_type == RESIZE_TYPE::MANUAL) {
            this->add_x_layer((newX - org_t_abs) /
                              2); // adding layers to the +ve x axis side
            this->inject_x_layer((newX - org_t_abs) -
                                 (newX - org_t_abs) /
                                     2); // adding layers to the -ve x axis side
        } else { // using different behaviour in Auto Expansion, so that
                 // layers/planes don't just get added only in direction (for
                 // eg. only in +ve direction, this happens when each time
                 // increment is only of 1 units)
    ...
    if (this->total_abs.mY < newY) {
        if (tmp_resize_data.curr_resize_type == RESIZE_TYPE::MANUAL) {
            this->add_y_layer((newY - org_t_abs) /
                              2); // adding a layer to the +ve y axis side
            this->inject_y_layer(
                (newY - org_t_abs) -
                (newY - org_t_abs) /
                    2); // adding a layer to the -ve y axis side
    ...
    if (resize_called_again)
        std::cout << "Already resizing" << std::endl;
    ...
    if (this->total_abs.mZ < newZ) {
        if (tmp_resize_data.curr_resize_type == RESIZE_TYPE::MANUAL) {
            this->add_z_layer((newZ - org_t_abs) /
                              2); // adding a layer to the +ve z axis side
            this->inject_z_layer(
                (newZ - org_t_abs) -
                (newZ - org_t_abs) /
                    2); // adding a layer to the -ve z axis side
    ...
    if (resize_type == RESIZE_TYPE::MANUAL) {
        if (was_auto_expanding) {
            ...
            // we `start` it again on another thread (with previous expansion
            // metadata still in __expansion_state object)
            std::thread(&Graph_Matrix_3D::auto_expansion, this).detach();
        }
    } else {
        tmp_resize_data.add_or_inject_flag =
            !tmp_resize_data
                 .add_or_inject_flag; // toggle it, so next time, planes get
                                      // added in alternative fashion (equally
...
template <typename node_dtype, typename dimen_t>
inline std::tuple<dimen_t, dimen_t, dimen_t>
Graph_Matrix_3D<node_dtype, dimen_t>::get_size() const noexcept {
    return {this->total_abs.mX, this->total_abs.mY, this->total_abs.mZ};
}

template <typename node_dtype, typename dimen_t>
inline std::array<std::pair<dimen_t, dimen_t>, 3>
Graph_Matrix_3D<node_dtype,
                dimen_t>::_implementation_detail_get_dimension_bounds()
    const noexcept {
    return {std::make_pair(this->min_x, this->max_x),
...
inline void Graph_Matrix_3D<node_dtype, dimen_t>::for_each(
    graph_box_type *source, Direction dir, Func func) noexcept {
...
template <typename Func>
inline void Graph_Matrix_3D<node_dtype, dimen_t>::for_each(
    graph_box_type *begin, graph_box_type *end, Direction dir,
    Func func) noexcept {
    static_assert(std::is_invocable_v<Func, const node_dtype &>,
                  "The Callable expected by for_each must take only "
                  "`node_dtype&` as input");
    ...
    while (tmp != end) {
        ...
        if (tmp->coordinate.mX < min_x || tmp->coordinate.mX > max_x ||
            tmp->coordinate.mY < min_y || tmp->coordinate.mY > max_y ||
            tmp->coordinate.mZ < min_z || tmp->coordinate.mZ > max_z) {
            throw std::logic_error(
                "Begin and End passed to Graph_Matrix_3D::for_each() not "
                "connected !!");
        }
#endif
        func(tmp->data); // depends on func whether it takes by reference or not
...
inline void Graph_Matrix_3D<node_dtype, dimen_t>::for_all(_Func func) {
    graph_box_type *x_temp{this->top_left_front}, *y_temp{this->top_left_front},
        *z_temp{this->top_left_front};

    /*at this->top_left_front, only x is at minimum, while y and z coords are
     * maximum there*/
    dimen_t x_coord{min_x}, y_coord{max_y}, z_coord{max_z};
...
template <typename node_dtype, typename dimen_t>
Graph_Box_3D<node_dtype> *Graph_Matrix_3D<node_dtype, dimen_t>::find(
    const node_dtype &value) { // uses operator== for comparison
    // @future - Use the top positions tree to chose from the best positions

    // @note- For small matrices, there is a speed tradeoff using threads
    // actually
...
// Graph_Box_3D<node_dtype>* Graph_Matrix_3D<node_dtype,
// dimen_t>::find(UnaryPredicate& func) {
//	static_assert(std::is_invocable_r_v<bool, UnaryPredicate, const
//node_dtype&>, 		"UnaryPredicate only considers functions that take in a `const
//node_dtype&` reference, and return a boolean value");;
//
//	// @future - Use the top positions tree to chose from the best positions
//
...
template <typename node_dtype, typename dimen_t>
Graph_Box_3D<node_dtype> *Graph_Matrix_3D<node_dtype, dimen_t>::swastic_find(
    graph_box_type *center_node,
    const node_dtype &value) { // uses operator== for comparison
    if (center_node->data == value)
        return center_node;

    std::vector<std::thread>
        layer_threads; // all threads that are searching different layers
    ...
    auto swastic = [&](graph_box_type *layer_center) mutable {
        ...
        graph_box_type *up{layer_center->UP}, *down{layer_center->DOWN},
            *left{layer_center->LEFT}, *right{layer_center->RIGHT};
    ...
    graph_box_type *tmp1{center_node->FRONT_FACING},
        *tmp2{center_node->BACK_FACING};
    ...
    while (!found) {
        // if no thread is joinable (ie. all have done executing) have done
        // executing and we didn't find, then exit
        if (std::none_of(layer_threads.begin(), layer_threads.end(),
                         [](const auto &th) { return th.joinable(); }))
            break;
...
// template<typename UnaryPredicate>
// Graph_Box_3D<node_dtype>* Graph_Matrix_3D<node_dtype,
// dimen_t>::swastic_find(graph_box_type* center_node, UnaryPredicate& func) {
//	static_assert(std::is_invocable_r_v<bool, UnaryPredicate, const
//node_dtype&>, 		"UnaryPredicate only considers functions that take in a `const
//node_dtype&` reference, and return a boolean value");;
//
...
template <typename node_dtype, typename dimen_t>
inline void
Graph_Matrix_3D<node_dtype, dimen_t>::expand_n_unit(const uint8_t n) {
    if (n <= 0)
        return;

    this->resize(this->total_abs.mX + n, this->total_abs.mY + n,
                 this->total_abs.mZ + n,
                 RESIZE_TYPE::AUTO_EXPANSION // this method is ONLY meant to be
                                             // used by auto expansion
    );
}

template <typename node_dtype, typename dimen_t>
inline void Graph_Matrix_3D<node_dtype, dimen_t>::add_x_layer(
    int num) // adds to end of xz planes (towards end of +x axis)
{
    // @note - Currently there is a LOT of duplicacy due to the add_*_layer
    // being repeated for all three directions, i plan to use template to ease
    // this, though it will require a few more if statements inside loops, but
    // depending on compiler to pre inline on basis of a template parameter (of
    // a type like DIRECTION etc), won't be doing that for now though, will do
    ...
    const bool use_initialiser =
        this->tmp_resize_data.curr_resize_type == RESIZE_TYPE::MANUAL
            ? this->data_initialiser.has_value()
            : this->__expansion_state.initializer_function.has_value();
    ...
    graph_box_type *tmp_y{this->top_right_front}, *tmp_z{this->top_right_front},
    ...
    for (auto i = 0; i < num; i++) {
        ...
        while (tmp_z) {
            ...
            do {
                ...
                if (use_initialiser) {
                    switch (
                        init_function->index()) // @caution - When Init_Function
                                                // typedef is modified, this
                    ...
                    case 1:
                        curr_new->data =
                            std::get<1>(*init_function)(max_x + 1, y, z);
                        break;
                    case 2:
                        std::get<2> (*init_function)(curr_new->data, max_x + 1,
                                                     y, z);
                        break;
                    case 3:
                        ...
                        break;
                    default:
                        // @warning - This path NOT implemented, if it's ever
                        // reached
                        break;
                    }
                }
                ...
                curr_new->LEFT =
                    tmp_y; // all other pointers are automatically nullptr
                ...
                if (tmp_z_curr_prev) { // will only fail for the first vertial
                                       // line (parallel to y-axis is added the
                                       // the new layer formed)
...
inline void Graph_Matrix_3D<node_dtype, dimen_t>::inject_x_layer(int num) {
    ...
    graph_box_type *tmp_y{this->top_left_front}, *tmp_z{this->top_left_front},
        *tmp_z_curr_prev{nullptr}, // just before the current new box
        *tmp_y_curr_prev{nullptr}; // just above the current new box
...
inline void Graph_Matrix_3D<node_dtype, dimen_t>::pop_xplus_layer() {
    if (total_abs.mX == 0 || max_x == 0)
        return; // don't pop the layer with origin in it
    ...
    do {
        ...
        do {
            ...
            y_tmp->LEFT->RIGHT =
                nullptr; // clearing pointers of previous level to use
            y_tmp = y_tmp->DOWN;

            // delete to_delete;
        } while (y_tmp);

        // if (z_tmp != to_delete)	// in case when there is only single
        // plane xy, and no other parallel, then z_tmp and y_tmp will be equal,
        // so when y_tmp deleted, z_tmp still points to it, and considers the
        // garbage value, and continue to prevent that 	z_tmp =
        //z_tmp->BACK_FACING; else z_tmp = nullptr;
        z_tmp = z_tmp_next;
...
inline void Graph_Matrix_3D<node_dtype, dimen_t>::pop_xminus_layer() {
    ...
    // strategy same as pop_xplus_layer, just so we use RIGHT instead of LEFT,
    // AND TOP_LEFT instead of TOP_RIGHT
    if (total_abs.mX == 0 || min_x == 0)
        return; // don't pop the layer with origin in it
    ...
    while (z_tmp) {
        ...
        while (y_tmp) {
            y_tmp->RIGHT->LEFT =
                nullptr; // clearing pointers of previous level to us

            to_delete = y_tmp;
            y_tmp = y_tmp->DOWN;

            // delete to_delete;	// DON'T DELETE IT NOW, THIS IS A TRADEOFF !
            // Memory overhead will be less due to less allocations, but at the
            // same time old memory won't be used again, and will remain as such
            // and removed in destructor only, so this approach is likely only
            // helpful for EXPANDING MATRICES
        }
...
inline void Graph_Matrix_3D<node_dtype, dimen_t>::add_y_layer(int num) {
    ...
    // both starting at top_left_front
    if (num < 1)
        return;
    ...
    graph_box_type *tmp_x{this->top_left_front}, *tmp_z{this->top_left_front},
        *tmp_z_curr_prev{nullptr}, // just before the current new box
        ...
        while (tmp_z) {
            ...
            do {
                ...
                if (use_initialiser) {
                    ...
                    case 1:
                        curr_new->data =
                            std::get<1>(*init_function)(x, max_y + 1, z);
                        break;
                    case 2:
                        std::get<2> (*init_function)(curr_new->data, x,
                                                     max_y + 1, z);
                        break;
                    case 3:
                        std::get<3> (*init_function)(*curr_new);
                ...
                curr_new->DOWN =
                    tmp_x; // all other pointers are automatically nullptr
                curr_new->FRONT_FACING = tmp_z_curr_prev;
                ...
                x_prev_new_box =
                    curr_new; // @important @note -> temp_x_prev and temp_z_prev
                              // WILL be on the new layer itself, don't equate
                              // with tmp_y or tmp_<.> since it's on the
                              // previous layer
            ...
            x_prev_new_box = nullptr; // for the first new block at min_x, there
                                      // won't be a prev element

            tmp_z_curr_prev = tmp_z->UP; // now the tmp_z_curr_prev will be on
                                         // the new layer formed
            tmp_z = tmp_z->BACK_FACING;
            tmp_x = tmp_z;
...
template <typename node_dtype, typename dimen_t>
inline void
Graph_Matrix_3D<node_dtype, dimen_t>::inject_y_layer(int number_of_layers) {
    // @brief tmp_z goes towards -ve z axis, and tmp_x goes towards +ve x axis
    if (number_of_layers < 1)
        return;
    const int num_box_required =
        number_of_layers * (total_abs.mX * total_abs.mZ);
        ...
        while (tmp_z) {
            ...
            do {
                ...
                if (use_initialiser) {
                    ...
                    case 1:
                        curr_new->data =
                            std::get<1>(*init_function)(x, min_y - 1, z);
                        break;
                    case 2:
                        std::get<2> (*init_function)(curr_new->data, x,
                                                     min_y - 1, z);
...
inline void Graph_Matrix_3D<node_dtype, dimen_t>::pop_yminus_layer() {
    ...
    if (total_abs.mY == 0 || min_y == 0)
        return; // don't pop the layer with origin in it

    graph_box_type *x_tmp{this->bottom_left_front},
        *z_tmp{this->bottom_left_front};
    // z_tmp will move towards -ve z-axis
    // and for each such move, x_tmp will move towards +ve x-axis
    ...
    do {
        ...
        // garbage value, and continue to prevent that 	z_tmp =
        //z_tmp->BACK_FACING; else z_tmp = nullptr;
        z_tmp = z_tmp_next; // in case z_tmp had been equal to x_tmp, ie. only 1
                            // layer would have been present, then that node is
                            // now freed, and we have garbage value, which we
                            // shouldn't dereference to z_tmp->BACK_FACING;
...
inline void Graph_Matrix_3D<node_dtype, dimen_t>::add_z_layer(int num) {
    // @brief -> first tmp_y goes from MAX_y to MIN_y (towards -ve y), and for
    // each such tmp_y, tmp_x moves from min_x to max_x (towards +x axis)
    if (num < 1)
        return;
    ...
    graph_box_type *tmp_x{this->top_left_front}, *tmp_y{this->top_left_front},
        *tmp_x_curr_prev{nullptr}, // just before the current new box
        *tmp_y_curr_prev{nullptr}; // just above the current new box

    int index = 0; // index in the memory above (num_box_required)
    //	@ assert that total number of times these loops run doesn't exceed
    //`num_box_required`
        ...
        while (tmp_y) {
            ...
            do {
                ...
                if (use_initialiser) {
                    ...
                    case 1:
                        curr_new->data =
                            std::get<1>(*init_function)(x, y, max_z + 1);
                        break;
                    case 2:
                        std::get<2> (*init_function)(curr_new->data, x, y,
                                                     max_z + 1);
                        break;
                    case 3:
                        std::get<3> (*init_function)(*curr_new);
                ...
                if (tmp_y_curr_prev) { // will only fail for the first vertial
                                       // line (parallel to y-axis is added the
                                       // the new layer formed)
            ...
            tmp_x_curr_prev = nullptr; // there isn't anything preeceding the
                                       // 1st element (in loop when it enters)
...
inline void Graph_Matrix_3D<node_dtype, dimen_t>::inject_z_layer(int num) {
    ...
    graph_box_type *tmp_x{this->top_left_back}, *tmp_y{this->top_left_back},
        *tmp_x_curr_prev{nullptr}, // just before the current new box
        *tmp_y_curr_prev{nullptr}; // just above the current new box
        ...
        while (tmp_y) {
            ...
            do {
                ...
                if (use_initialiser) {
                    ...
                    case 1:
                        curr_new->data =
                            std::get<1>(*init_function)(x, y, min_z - 1);
                        break;
                    case 2:
                        std::get<2> (*init_function)(curr_new->data, x, y,
                                                     min_z - 1);
...
inline void Graph_Matrix_3D<node_dtype, dimen_t>::pop_zplus_layer() {
    ...
    do {
        ...
        do {
            y_tmp->BACK_FACING->FRONT_FACING =
                nullptr; // clearing pointers of previous level to us
        ...
        // if (x_tmp != to_delete)	// in case when there is only single
        // plane xy, and no other parallel, then z_tmp and y_tmp will be equal,
        // so when y_tmp deleted, z_tmp still points to it, and considers the
        // garbage value, and continue to prevent that 	x_tmp = x_tmp->RIGHT; else
        // x_tmp = nullptr;
        x_tmp = x_tmp_next;
        y_tmp = x_tmp;
    } while (x_tmp);
...
inline void Graph_Matrix_3D<node_dtype, dimen_t>::pop_zminus_layer() {
    ...
    do {
        ...
        do {
            y_tmp->FRONT_FACING->BACK_FACING =
                nullptr; // clearing pointers of previous level to us
...
template <typename node_dtype, typename dimen_t>
inline void Graph_Matrix_3D<node_dtype, dimen_t>::disp_xy_layer(
    MatrixLayer ltype) const noexcept {
    switch (ltype) {
...
template <typename node_dtype, typename dimen_t>
inline void Graph_Matrix_3D<node_dtype, dimen_t>::disp_xy_layer(
    int z_lnum, std::ostream &os) const noexcept {
    if (z_lnum < min_z || z_lnum > max_z)
        return disp_xy_layer(MatrixLayer::TOP);
    ...
    graphMat::DirectionalIterator<node_dtype, Direction::PURVA, true> y_iter(
        start_pos);
    graphMat::DirectionalIterator<node_dtype, Direction::DAKSHIN, true> x_iter(
        start_pos);
 ...
 *
 * @note2 - This implementation calls expand_once to decide the expansion EACH
 * 500 milliseconds (ie. 0.5 seconds)
 */
template <typename node_dtype, typename dimen_t>
inline void Graph_Matrix_3D<node_dtype, dimen_t>::auto_expansion() {
    ...
    while (this->__expansion_state.expansion_flag) {
        ...
        std::this_thread::sleep_for(std::chrono::milliseconds(
            this->__expansion_state.milliseconds_in_unit_time));
    }
...
inline void Graph_Matrix_3D<node_dtype, dimen_t>::pause_auto_expansion() {
    ...
    } // block till auto_expansion doesn't signal that it's done

    // while (this->__expansion_state.expansion_flag)	// to prevent infinite
    // blocking 	this->auto_expansion_convar.wait_for(lock,
...
template <typename node_dtype, typename dimen_t>
inline void
Graph_Matrix_3D<node_dtype, dimen_t>::set_expansion_rate(float rate) noexcept {
...
inline void Graph_Matrix_3D<node_dtype, dimen_t>::resume_auto_expansion(
    Callable &&inititaliser_func) { // callable receives the graph_box reference
    static_assert(std::is_assignable_v<Init_Func, Callable>,
                  "The callable MUST have EITHER of these signatures - "
                  "void(graph_box_type&) or void(node_dtype, dimen_t, dimen_t, "
                  "dimen_t)   or   node_dtype(dimen_t, dimen_t, dimen_t)  "
                  "[with the inside parenthesis being the parameters taken, "
                  "and outside being the return type]");
...
template <typename node_dtype, typename dimen_t>
inline Graph_Box_3D<node_dtype> *
Graph_Matrix_3D<node_dtype, dimen_t>::operator[](const coord_type &pos) {
...
template <typename node_dtype, typename dimen_t>
inline const Graph_Box_3D<node_dtype> *
Graph_Matrix_3D<node_dtype, dimen_t>::operator[](const coord_type &pos) const {
    return const_cast<Graph_Matrix_3D<node_dtype, dimen_t> *>(this)->operator[](
        pos);
}

template <typename node_dtype, typename dimen_t>
inline Graph_Box_3D<node_dtype> *
Graph_Matrix_3D<node_dtype, dimen_t>::operator[](const graph_position &pos) {
    // @note to viewer -> You can express your view on whether we should prefer
    // simple [x][y] for position or the graph_position typedefed in
    // graph_box.hpp
...
template <typename node_dtype, typename dimen_t>
inline const Graph_Box_3D<node_dtype> *
Graph_Matrix_3D<node_dtype, dimen_t>::operator[](
    const graph_position &pos) const {

    /*Suggested in Effective C++, instead of writing the same code in both the
     * const and non-const duplicate member functions*/
...
inline Graph_Matrix_3D<node_dtype, dimen_t>::Graph_Matrix_3D()
    : Graph_Matrix_3D({1, 1, 1}) {}

template <typename node_dtype, typename dimen_t>
inline Graph_Matrix_3D<node_dtype, dimen_t>::Graph_Matrix_3D(
    const coord_type &dimensions)
    : Matrix_Base(3),
      total_abs({1, 1, 1}), /*absolute_ values aren't max-min, since at
                               beginning min=max=0, but absolute is 1*/
      top_left_front(&origin), top_left_back(&origin),
      bottom_left_front(&origin), bottom_left_back(&origin),
      top_right_front(&origin), top_right_back(&origin),
      bottom_right_front(&origin), bottom_right_back(&origin), min_x(0),
      min_y(0), min_z(0), max_x(0), max_y(0), max_z(0) {
...
template <typename Func>
inline Graph_Matrix_3D<node_dtype, dimen_t>::Graph_Matrix_3D(
    const coord_type &dimensions, Func &&initialiser)
    : // with initialiser
      Matrix_Base(3),
      total_abs({1, 1, 1}), /*absolute_ values aren't max-min, since at
                               beginning min=max=0, but absolute is 1*/
      ...
      bottom_right_front(&origin), bottom_right_back(&origin), min_x(0),
      min_y(0), min_z(0), max_x(0), max_y(0), max_z(0) {
    static_assert(std::is_assignable_v<Init_Func, Func>,
                  "The callable MUST have EITHER of these signatures - "
                  "void(graph_box_type&) or void(node_dtype, dimen_t, dimen_t, "
                  "dimen_t)   or   node_dtype(dimen_t, dimen_t, dimen_t)  "
                  "[with the inside parenthesis being the parameters taken, "
    ...
    this->resize(dimensions.mX, dimensions.mY, dimensions.mZ, initialiser,
                 RESIZE_TYPE::MANUAL);
}

template <typename node_dtype, typename dimen_t>
inline Graph_Matrix_3D<node_dtype, dimen_t>::~Graph_Matrix_3D() {
    // actually wanted to use promise, so that we wait till the promise is set
    // by pause_auto_expansion, but then we can't make the destructor noexcept
    // if that matters, if i am seeing this you will consider this possibility
    // :D
